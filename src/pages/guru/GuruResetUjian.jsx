...
export default function GuruResetUjian() {
  ...
  const fetchData = async () => {
    try {
      // Hanya ambil sesi ujian yang guru_id-nya adalah user saat ini
      const { data: res, error } = await supabase
        .from('ujian_aktif')
        .select(`
          *,
          profiles (nama_lengkap, username),
          jadwal_ujian (nama_ujian, token, guru_id, bank_soal_id)
        `)
        .eq('jadwal_ujian.guru_id', profile.id)
        .order('created_at', { ascending: false });
      
      if (error) throw error;
      
      // Karena .eq() pada joined table di Supabase JS kadang tidak memfilter root, kita filter manual jika perlu
      const filteredRes = res?.filter(item => item.jadwal_ujian?.guru_id === profile.id);
  ...
  return (
    <div className="min-h-full p-4 md:p-8 animate-in fade-in duration-500 bg-[#f8fafc]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">Reset Login Siswa</h1>
          <p className="text-slate-500 text-[13px] font-medium">Buka blokir siswa atau selesaikan paksa ujian siswa Anda</p>
        </div>
      </div>
